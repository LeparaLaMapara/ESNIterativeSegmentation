...
class CRNN(nn.Module):
    def __init__(self, in_channels=3, sample_size=256, sample_duration=16, num_classes=100,
        ...
        self.k1, self.k2, self.k3, self.k4 = (7, 7), (7, 7), (5, 5), (5, 5)
    ...
    def forward(self, x):
        ...
        for t in range(x.size(2)):
            ...
            # print('x in', x.shape)
            out = self.conv1(x[:, :, t, :, :])
            # print('conv1', out.shape, out.view(out.size(0), -1).shape)
            out = self.conv2(out)
            # print('conv2', out.shape, out.view(out.size(0), -1).shape)
            # out = self.conv3(out)
            # print('conv3', out.shape, out.view(out.size(0), -1).shape)
            # out = self.conv4(out)
            # print('conv4', out.shape, out.view(out.size(0), -1).shape)
            # print(out.shape)
