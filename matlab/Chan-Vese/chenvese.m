...
function seg = chenvese(imagename,path_destination, mask,num_iter,mu,method)




% load image
I = imread(string(imagename));
    ...
    case 'multiphase'
       ...
       % subplot(2,2,1); 
        ...
        for n=1:num_iter
              ...
              imwrite(ind2rgb(im2uint8(mat2gray(seg)), parula(256)), filename)
              disp(filename)
              %intermediate output
              %if(mod(n,20) == 0) 
                % phi(:,:,1) = phi1;
